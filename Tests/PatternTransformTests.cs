...
namespace WootMouseRemap.Tests
{
    ...
    {
        private AntiRecoilPattern _testPattern;
        ...
        public void Setup()
        {
            ...
            {
                Name = "TestPattern",
                Samples = new List<RecoilSample>
                {
                    new RecoilSample { Timestamp = TimeSpan.FromMilliseconds(0), Dx = 0, Dy = 10 },
                    new RecoilSample { Timestamp = TimeSpan.FromMilliseconds(16), Dx = 2, Dy = 8 },
                    new RecoilSample { Timestamp = TimeSpan.FromMilliseconds(32), Dx = -1, Dy = 12 },
                    new RecoilSample { Timestamp = TimeSpan.FromMilliseconds(48), Dx = 1, Dy = 6 },
                    new RecoilSample { Timestamp = TimeSpan.FromMilliseconds(64), Dx = 0, Dy = 4 },
                    new RecoilSample { Timestamp = TimeSpan.FromMilliseconds(80), Dx = -2, Dy = 2 }
                }
            };
        }
        ...
        public void PatternTransforms_Normalize_WithZeroPeak_ShouldReturnOriginal()
        {
            var zeroPattern = new AntiRecoilPattern
            {
                ...
                Samples = new List<RecoilSample>
                {
                    new RecoilSample { Timestamp = TimeSpan.Zero, Dx = 0, Dy = 0 }
                }
            };
        ...
        public void PatternTransforms_SmoothGaussian_ShouldPreserveSampleCount()
        {
            ...
            Assert.AreEqual(_testPattern.Samples.Count, smoothed.Samples.Count, "Sample count should be preserved");
            Assert.AreEqual(_testPattern.Name + "_Gaussian", smoothed.Name, "Name should be updated");
        }
        ...
        public void PatternTransforms_Trim_ShouldRemoveLeadingAndTrailingZeros()
        {
            var patternWithZeros = new AntiRecoilPattern
            {
                ...
                Samples = new List<RecoilSample>
                {
                    new RecoilSample { Timestamp = TimeSpan.FromMilliseconds(0), Dx = 0, Dy = 0 },
                    new RecoilSample { Timestamp = TimeSpan.FromMilliseconds(16), Dx = 0, Dy = 0 },
                    new RecoilSample { Timestamp = TimeSpan.FromMilliseconds(32), Dx = 2, Dy = 8 },
                    new RecoilSample { Timestamp = TimeSpan.FromMilliseconds(48), Dx = 1, Dy = 6 },
                    new RecoilSample { Timestamp = TimeSpan.FromMilliseconds(64), Dx = 0, Dy = 0 },
                    new RecoilSample { Timestamp = TimeSpan.FromMilliseconds(80), Dx = 0, Dy = 0 }
                }
            };

            var trimmed = PatternTransforms.Trim(patternWithZeros, 1.0f);

            Assert.AreEqual(2, trimmed.Samples.Count, "Should keep only non-zero samples");
            Assert.AreEqual(8, trimmed.Samples.First().Dy, "First sample should be first non-zero");
            Assert.AreEqual(6, trimmed.Samples.Last().Dy, "Last sample should be last non-zero");
        }
        ...
        public void PatternTransforms_Invert_ShouldNegateYValues()
        {
            ...
            for (int i = 0; i < _testPattern.Samples.Count; i++)
            {
                ...
                Assert.AreEqual(original.Timestamp, invertedSample.Timestamp, "Timestamps should not change");
            }
        }
        ...
        public void PatternTransforms_RemoveOutliers_ShouldFilterExtremeValues()
        {
            var patternWithOutliers = new AntiRecoilPattern
            {
                ...
                Samples = new List<RecoilSample>
                {
                    new RecoilSample { Timestamp = TimeSpan.FromMilliseconds(0), Dx = 0, Dy = 5 },
                    new RecoilSample { Timestamp = TimeSpan.FromMilliseconds(16), Dx = 1, Dy = 6 },
                    new RecoilSample { Timestamp = TimeSpan.FromMilliseconds(32), Dx = 2, Dy = 50 }, // Outlier
                    new RecoilSample { Timestamp = TimeSpan.FromMilliseconds(48), Dx = 0, Dy = 4 },
                    new RecoilSample { Timestamp = TimeSpan.FromMilliseconds(64), Dx = -1, Dy = 5 }
                }
            };
        ...
        {
            var stats = PatternTransforms.CalculateStatistics(_testPattern);

            Assert.AreEqual(_testPattern.Samples.Count, stats.SampleCount, "Sample count should match");
            Assert.AreEqual(12, stats.MaxDy, "Max Y should be correct");
            Assert.AreEqual(2, stats.MinDy, "Min Y should be correct");

            var expectedAvgDy = _testPattern.Samples.Average(s => s.Dy);
            Assert.AreEqual(expectedAvgDy, stats.AvgDy, 0.01f, "Average Y should be correct");

            var duration = _testPattern.Samples.Last().Timestamp - _testPattern.Samples.First().Timestamp;
            Assert.AreEqual(duration, stats.Duration, "Duration should be correct");
        }
        ...
        public void PatternTransforms_WithEmptyPattern_ShouldHandleGracefully()
        {
            ...
            var trimmed = PatternTransforms.Trim(emptyPattern, 1.0f);
        ...
        public void PatternTransforms_WithSingleSample_ShouldHandleGracefully()
        {
            var singleSamplePattern = new AntiRecoilPattern
            {
                ...
                Samples = new List<RecoilSample>
                {
                    new RecoilSample { Timestamp = TimeSpan.Zero, Dx = 5, Dy = 10 }
                }
            };
