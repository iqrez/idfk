...
namespace WootMouseRemap.Tests
{
    ...
    public class AntiRecoilTests
    {
        ...
        public void Setup()
        {
            // Delete any existing settings file BEFORE creating AntiRecoil instance
            DeleteFileWithRetry("antirecoil_settings.json");
            
            _antiRecoil = new AntiRecoil();
            // Reset to defaults for consistent test state
            _antiRecoil.Enabled = false;
            _antiRecoil.Strength = 0.5f;
            _antiRecoil.VerticalThreshold = 2.0f;
            _antiRecoil.ActivationDelayMs = 0;
        ...
        private void DeleteFileWithRetry(string path, int maxRetries = 5, int delayMs = 50)
        {
        ...
        public void AntiRecoil_EnabledWithZeroStrength_ShouldNotCompensate()
        {
            ...
            Assert.AreEqual(5, result.dy, "No vertical compensation should occur with zero strength (output should equal input)");
        }
        ...
        public void AntiRecoil_MovementBelowThreshold_ShouldNotActivate()
        {
            ...
            Assert.AreEqual(2, result.dy, "No compensation should occur below threshold (output should equal input)");
        }
        ...
        public void AntiRecoil_MovementAboveThreshold_ShouldCompensate()
        {
            ...
            // Start shooting detection
            _antiRecoil.OnShootingStarted();

            _antiRecoil.Enabled = true; // Set again after OnShootingStarted

            // Debug state
            Console.WriteLine($"Enabled: {_antiRecoil.Enabled}, IsShootingDetected: {_antiRecoil.IsShootingDetected}, IsActive: {_antiRecoil.IsActive}");

            var result = _antiRecoil.ProcessMouseMovement(0, -10); // Upward movement (recoil) above threshold

            // Debug output
            Console.WriteLine($"Input dy: -10, Result dy: {result.dy}, Expected: > -10");

            Assert.AreEqual(0, result.dx, "No horizontal compensation expected");
            Assert.IsTrue(result.dy > -10, "Vertical compensation should reduce upward movement (less negative)");
        ...
        public void AntiRecoil_MaxTickCompensation_ShouldCapPerTickCompensation()
        {
            ...
            var result = _antiRecoil.ProcessMouseMovement(0, -20); // Large upward movement

            var compensation = -result.dy - (-20); // Calculate applied compensation (result.dy = input.dy - compensation)
            Assert.IsTrue(compensation <= 5.0f, "Per-tick compensation should not exceed MaxTickCompensation");
        }
        ...
        public void AntiRecoil_CooldownPeriod_ShouldPreventImmediateReactivation()
        {
            ...
            // Stop shooting briefly to reset cooldown
            _antiRecoil.OnShootingStopped();
            System.Threading.Thread.Sleep(50); // Wait less than cooldown
            _antiRecoil.OnShootingStarted();
        ...
        public void AntiRecoil_HorizontalCompensation_ShouldCompensateHorizontalMovement()
        {
            ...
            // Check that horizontal compensation applied is within bounds
            var hCompApplied = 8 - result.dx; // input.dx - result.dx
            Assert.IsTrue(Math.Abs(hCompApplied) <= Math.Abs(8 * 0.3f), "Horizontal compensation applied should not exceed input * horizontal compensation factor");
        }
        ...
        public void AntiRecoil_PatternRecording_ShouldCaptureMovements()
        {
            ...
            Assert.IsTrue(_antiRecoil.IsRecordingPattern, "Should be in recording mode");

            // Start shooting detection for recording
            _antiRecoil.OnShootingStarted();
            ...
            _antiRecoil.StopPatternRecording(true);
            Assert.IsFalse(_antiRecoil.IsRecordingPattern, "Should not be in recording mode");
