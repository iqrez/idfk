...
namespace WootMouseRemap
{
    public sealed class RawInput : IDisposable
    {
        ...
        public void Unregister()
        {
            ...
            try
            {
                ...
                if (!RegisterRawInputDevices(rid, (uint)rid.Length, (uint)Marshal.SizeOf<RAWINPUTDEVICE>()))
                {
                    ...
                    Logger.Warn("Failed to unregister raw input devices: {ErrorCode}", errorCode);
                }
                else
                {
                    _registered = false;
        ...
        private unsafe void ProcessRawInput(IntPtr hRawInput)
        {
            ...
            fixed (byte* p = buffer)
            {
                ...
                else if (header.dwType == RIM_TYPEMOUSE)
                {
                    ...
                    ProcessMouseButtons(mouse.usButtonFlags);
                    ...
                    if (dx != 0 || dy != 0)
                    {
                        // Remove excessive logging in hot path for performance
        ...
        private static readonly Dictionary<int, (MouseInput button, bool isDown)> MouseButtonMap = new()
        {
            { RI_MOUSE_BUTTON_1_DOWN, (MouseInput.Left, true) },
            { RI_MOUSE_BUTTON_1_UP, (MouseInput.Left, false) },
            { RI_MOUSE_BUTTON_2_DOWN, (MouseInput.Right, true) },
            { RI_MOUSE_BUTTON_2_UP, (MouseInput.Right, false) },
            { RI_MOUSE_BUTTON_3_DOWN, (MouseInput.Middle, true) },
            { RI_MOUSE_BUTTON_3_UP, (MouseInput.Middle, false) },
            { RI_MOUSE_BUTTON_4_DOWN, (MouseInput.XButton1, true) },
            { RI_MOUSE_BUTTON_4_UP, (MouseInput.XButton1, false) },
            { RI_MOUSE_BUTTON_5_DOWN, (MouseInput.XButton2, true) },
            { RI_MOUSE_BUTTON_5_UP, (MouseInput.XButton2, false) }
        };

        private void ProcessMouseButtons(ushort buttonFlags)
        {
            foreach (var (flag, (button, isDown)) in MouseButtonMap)
            {
                if ((buttonFlags & flag) != 0)
                    MouseButton?.Invoke(button, isDown);
            }
        }
