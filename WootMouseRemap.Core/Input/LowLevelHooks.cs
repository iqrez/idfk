...
namespace WootMouseRemap
{
    public static class LowLevelHooks
    {
        ...
        public static void Install(bool enableLowLevelHooks, bool complianceMode)
        {
            try
            {
                if (!enableLowLevelHooks || complianceMode) { Uninstall(); return; }
                ...
                if (_hkKb == IntPtr.Zero || _hkMs == IntPtr.Zero) 
                {
                    ...
                    Logger.Error("SetWindowsHookEx failed with error code: {ErrorCode}", error);
                ...
                if (nCode >= 0 && IsInstalled)
                {
                    ...
                    if (down || up) 
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            {
                                Logger.Error("Error in KeyEvent handler for key {KeyCode} (down: {IsDown})", kb.vkCode, down, ex);
                                _lastErrorLog = now;
                            }
                        }
                    }
            ...
            try
            {
                if (nCode >= 0 && IsInstalled)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        if (now - _lastErrorLog > TimeSpan.FromSeconds(5))
                        {
                            Logger.Error("Error in mouse event handler for message {Message}", msg, ex);
                            _lastErrorLog = now;
                        }
                    }
    ...
    private const int WM_LBUTTONDOWN = 0x0201, WM_LBUTTONUP = 0x0202, WM_RBUTTONDOWN = 0x0204, WM_RBUTTONUP = 0x0205,
        ...
        [DllImport("user32.dll", SetLastError = true, CharSet = CharSet.Unicode)] private static extern IntPtr SetWindowsHookEx(int idHook, LowLevelProc lpfn, IntPtr hMod, uint dwThreadId);
        [DllImport("user32.dll", SetLastError = true, CharSet = CharSet.Unicode)] private static extern bool UnhookWindowsHookEx(IntPtr hhk);
        [DllImport("user32.dll", SetLastError = true, CharSet = CharSet.Unicode)] private static extern IntPtr CallNextHookEx(IntPtr hhk, int nCode, IntPtr wParam, IntPtr lParam);
        ...
        [DllImport("user32.dll", SetLastError = true, CharSet = CharSet.Unicode)] private static extern short GetKeyState(int nVirtKey);
    }
}
