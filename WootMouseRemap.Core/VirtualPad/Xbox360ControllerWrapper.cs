    ...
    public sealed class Xbox360ControllerWrapper : IDisposable
    {
        ...
        public void Connect()
        {
            try
            {
                ...
                // 16ms = ~60Hz, sufficient for controller activity detection
                _watchdog = new global::System.Threading.Timer(_ => Submit(), null, 0, 16);
        ...
        public void SetDpad(bool up, bool down, bool left, bool right)
        {
            ...
            {
                Logger.Error("Xbox360ControllerWrapper: Error in SetDpad", ex);
                Reconnect();
            }
        }
        ...
        public void ResetAll()
        {
            try
            {
                ...
                // Reset internal snapshot to match controller state
                _snap.LX = _snap.LY = _snap.RX = _snap.RY = 0;
                _snap.LT = _snap.RT = 0;
                _snap.A = _snap.B = _snap.X = _snap.Y = false;
                _snap.LB = _snap.RB = _snap.Back = _snap.Start = false;
                _snap.L3 = _snap.R3 = false;
                _snap.DUp = _snap.DDown = _snap.DLeft = _snap.DRight = false;
            }
            catch (Exception ex)
            {
        ...
        {
            // Reset any smoothing state - currently no smoothing in this wrapper
            // but method exists for interface compatibility
        }
    }
}
