...
namespace WootMouseRemap.UI
{
    ...
    public partial class AntiRecoilConfigForm : Form
    {
        ...
        private void SaveCurrentPattern()
        {
            ...
            {
                Logger.Error("Failed to save pattern changes: {Message}", ex.Message);
            }
        }
        ...
        private void CreateAutoBackups()
        {
            ...
            {
                Logger.Info("Auto-backup failed", ex);
            }
        }
        ...
        private void TelemetryTimer_Tick(object? sender, EventArgs e)
        {
            ...
            {
                var efficiency = (stats.TotalCompensation > 0 && stats.MaxVerticalMovement > 0) 
                    ? WootMouseRemap.Security.SecureArithmetic.SafeDivide(stats.AvgCompensation, stats.MaxVerticalMovement) * 100 
                    : 0;
                _telemetryStatsLabel.Text = $"Duration: {stats.TimeSpan.TotalSeconds:F1}s | Max Movement: {stats.MaxVerticalMovement:F1} | Total Compensation: {stats.TotalCompensation:F1} | Efficiency: {efficiency:F1}%";
            }
        ...
        private void LoadUIState()
        {
            ...
            {
                Logger.Info("Failed to load UI state", ex);
                _uiState = new UiStateData();
            }
        }
        ...
        private void ApplyUIState()
        {
            ...
            {
                Logger.Info("Failed to apply UI state", ex);
            }
        }
